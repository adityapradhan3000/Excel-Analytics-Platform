  ...
  const handleDelete = async (id) => {
    try {
      const res = await axios.delete(
        `${backendUrl}/api/chart/deleteChart/${id}`
      );
      if (res.data.success) {
  ...
  return (
    <div className="h-screen w-screen bg-gradient-to-tr from-gray-600 via-gray-700 to-gray-900 overflow-auto">
      ...
      <div className="flex flex-row justify-center items-center">
        ...
        className="animate-bounce mt-5 rounded-full shadow-xl shadow-orange-800 bg-gradient-to-r from-blue-400 via-orange-300 to-orange-400 px-7 py-2">
          <h1 className="font-extrabold text-lg">3D HISTORY</h1>
        </button>
      </div>
    </div>
  );
